{{-- resources/views/profile/partials/form-siswa.blade.php --}}
<div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div class="form-control col-span-2 md:col-span-1 w-full">
        ...
        <input type="text" name="kategori_umur" id="kategoriUmur" class="text-input text-input-neutral w-full border-gray-200 read-only:bg-gray-200"
            readonly value="{{ old('kategori_umur', $profile->kategori_umur ?? '') }}" {{ auth()->user()->hasRole('siswa') ? 'disabled' : '' }}>
    </div>

    {{-- Nomor Jersey --}}
    <div class="form-control col-span-2 md:col-span-1 w-full">
        <label class="label text-black text-bold mb-2 w-full">Nomor Jersey</label>
        <input type="text" name="nomor_jersey" class="text-input text-input-neutral w-full border-gray-200 read-only:bg-gray-200"
            value="{{ old('nomor_jersey', $profile->nomor_jersey ?? '') }}" {{ auth()->user()->hasRole('siswa') ? 'disabled' : '' }}>
    </div>
    ...
    <div class="form-control col-span-2 md:col-span-1 w-full">
        ...
        <input type="hidden" name="status_siswa" class="text-input text-input-neutral w-full border-gray-200 read-only:bg-gray-200" value="{{ old('status_siswa', optional($profile)->status_siswa ?? '') }}" {{ auth()->user()->hasRole('siswa') ? 'disabled' : '' }}>
    </div>

    {{-- Jenis Kelamin --}}
    <div class="form-control col-span-2 md:col-span-1 w-full">
        <!-- <label class="label text-black text-bold mb-2 w-full">Jenis Kelamin</label> -->
        <input type="hidden" name="jenis_kelamin" class="text-input text-input-neutral w-full border-gray-200 read-only:bg-gray-200" value="{{ old('jenis_kelamin', optional($profile)->jenis_kelamin ?? '') }}" {{ auth()->user()->hasRole('siswa') ? 'disabled' : '' }}>
    </div>
